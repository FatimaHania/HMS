...
function startSession(){
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            //styling the card
            $('#session_card'+session_id+' .card-header').attr('class','card-header text-dark bg-success');
            $('#session_card'+session_id+' #card_status_span').html('<button type="button" class="btn btn-xs btn-xing disabled">On-going</button>');
            $('#session_card'+session_id+' #card_cancel_span').html('');
            $('#session_card'+session_id+' #card_start_span').html('');
            $('#session_card'+session_id+' #card_complete_span').html('<button type="button" class="btn btn-xs btn-primary" id="complete_session_btn'+session_id+'" data-toggle="modal" data-target="#completeSessionModal" data-toggle="tooltip" data-placement="top" title="Complete Session" onclick="displaySessionCompleteForm('+session_id+')"><i class="fa fa-stop-circle" aria-hidden="true"></i></button>');
